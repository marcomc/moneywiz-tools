            ...
            if tool == "uv":
                if args == ["--version"]:
                    ...
                    header = (
                        ...
                        else f"# generated by: uv {shlex.join(args)}\\n"
                    )
                ...
                if args == ["--version"]:
                    ...
                    source = next(argument for argument in args if argument.endswith(".swift"))
                    executable(
...
def test_successful_reinstall_publishes_complete_bundle_and_command(
    ...
    assert (runtime / "doc/RECONCILIATION-READS.md").is_file()
...
def test_bundle_artifacts_do_not_disclose_build_paths(
    install_environment: dict[str, Path | dict[str, str]],
) -> None:
    result = _run_make(install_environment, "install")
    assert result.returncode == 0, result.stderr
